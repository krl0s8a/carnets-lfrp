...
class Assets {
    ...
    public static function external_js($extJs = null, $list = false, $addExtension = true, $bypassGlobals = false, $bypassInheritance = false) {
        ...
        if (!$renderSingleScript && !$list && self::$ci->config->item('assets.js_combine')
        ) {
    ...
    public static function assets_url($type = null, $modulePath = false) {
        ...
        if ($type !== null && $type !== 'base' && array_key_exists($type, self::$directories)
        ) {
...
<html><head><title>403 Forbidden</title></head><body><p>Directory access is forbidden.</p></body></html>'
    ...
    protected static function stripExtension($file, $type) {
        ...
        if (!isset($filePathInfo['extension']) || ".{$filePathInfo['extension']}" != $type
        ) {
            return $file;
        }

        return $filePathInfo['dirname'] != '.' ? self::path($filePathInfo['dirname'], $filePathInfo['filename']) : $filePathInfo['filename'];
    }
     ...
     * errors.
     */
    private static function generate_file($files = array(), string $fileName, $fileType = 'css') {
        if (count($files) == 0) {
    ...
    private static function find_files($files = array(), $type = 'css', $bypass_inheritance = false) {
        ...
        foreach ($files as &$file) {
            ...
            if (empty($module)) {
                ...
                $path = Module::file_path($module, self::$directories['base'], $file_path_name . $type);
                ...
                if (!empty($path) && $rtl_set) {
                    $path_rtl = Module::file_path($module, self::$directories['base'], "{$file_path_name}-{$rtl}{$type}");
                    if (!empty($path_rtl)) {
                        $path = $path_rtl;
                    }
                }
