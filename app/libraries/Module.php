...
is_array(Modules::$locations = $CFG->item('modules_locations')) ||
    ...
    realpath(APPPATH) . '/modules/' => '../../app/modules/'
);
...
class Module {
    ...
    public static function autoload($class) {
        ...
        if (strstr($class, 'MX_') && is_file($location = APPPATH . 'third_party/MX/' . substr($class, 3) . '.php')
        ) {
    ...
    public static function file_path($module = null, $folder = null, $file = null) {
        ...
        $ucFile = ucfirst($file);
        foreach (Module::folders() as $module_folder) {
    ...
    public static function files($module_name = null, $module_folder = null, $exclude_core = false) {
        ...
        foreach (Modules::folders() as $path) {
            ...
            if ($exclude_core && stripos($path, 'bonfire/modules') !== false
            ) {
                continue;
            }
