<div class="form-group">
    ...
    if (is_array($doc) && !empty($doc)) {
        ...
        foreach ($doc as $k => $v) {
            $s = false;
            if (is_array($personal_doc)) {
                $s = array_key_exists($k, $personal_doc);
            }
            ?>
            <li>
                <label>
                ...
                <input type="text" class="no-border date"
                    value="<?php echo ($s && $personal_doc[$k] != '0000-00-00') ? formatDate($personal_doc[$k], 'Y-m-d', 'd/m/Y') : ''; ?>"
                    name="due_date[<?php echo $k; ?>]" size="5%" placeholder="dd/mm/aaaa">

                <?php if (isset($personal_doc[$k]) && ($personal_doc[$k] != '0000-00-00') && ($personal_doc[$k]) < date('Y-m-d')): ?>
                    <span class="label label-danger">Vencido</span>
                <?php else: ?>
                    <?php if (isset($personal_doc[$k]) && $personal_doc[$k] != null): ?>
