...
class Personal_model extends MY_Model {
    ...
    public function insert($data = []) {
        ...
        $data['username'] = strtolower(str_replace(' ', '', $data['last_name'])) . substr($data['cuil'], 8, 3);
        $data['employee'] = 1;
    ...
    public function prep_data($post_data) {
        ...
        // Fechas
        if (isset($post_data['birth']) && !empty($post_data['birth'])) {
            $data['birth'] = formatDate($post_data['birth'], 'd/m/Y', 'Y-m-d');
        }
        if (isset($post_data['dateemployment']) && !empty($post_data['dateemployment'])) {
            $data['dateemployment'] = formatDate($post_data['dateemployment'], 'd/m/Y', 'Y-m-d');
        }
        if (isset($post_data['dateemploymentend']) && !empty($post_data['dateemploymentend'])) {
            $data['dateemploymentend'] = formatDate($post_data['dateemploymentend'], 'd/m/Y', 'Y-m-d');
        }
